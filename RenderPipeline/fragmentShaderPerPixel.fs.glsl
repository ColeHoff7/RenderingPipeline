...
vec3 localIllumination(GeneralLight light, Material object )
{
	...
	if(light.enabled == 1){
		...
		if(light.isSpot == 1){
	        float falloff = pow(light.spotCutoffCos, light.spotExponent);
			totalFromThisLight *= falloff;
		}
...
void main()
{
    ...
    for(int i = 0; i < MaxLights; i++){
	  fragmentColor += vec4(localIllumination(lights[i], object), 0.0f);
	}
	fragmentColor.a = object.ambientMat.a;
